...
class BayesianNonparametricNMF():
    ...
    def fit(self, X, W1, NbSteps=1, verbose=0):
        ...
        for step, x in enumerate(X):
            ...
            H2 = (self.h02 + np.sum(self.EW*(self.A1/self.A2),0)[:,np.newaxis])/T
            ...
            A1_up = self.a01 + (self.N_eff/N_batch) * (X_reshape*P).sum(2).sum(1).T#[np.newaxis,...]
            A2_up = self.a02 + (self.N_eff/N_batch) * (W1/W2).sum(0)*(H1/H2).sum(1).T
            ...
            if verbose > 0:
                print("step {}/{}, num patterns: {}".format(step+1, NbSteps, self.A1.shape[1]))
...
class BayesianHMM():
    ...
    def fit(self, X, EW, B1, NbSteps=1):
        ...
        tauPpi = .01/self.num_state
        tauA = .1/self.num_state
        tau1 = .1
        tau2 = .1

        n = 1

        B2 = (N/n)*1000*.03*np.ones((self.num_state,num_pat))

        for step, Xi in enumerate(X):
            print('Current step is {}/{}'.format(step, NbSteps))
            n = int(np.round(1 + 10*.75**(step-1)))
    ...
    def getFingerprints(self, As, Ppis):
        ...
        for i in range(As.shape[0]):
            ...
            statvec = Pinorm.T @ inv(np.eye(Ppi_len)-T/(T+1)*Anorm)@((np.eye(Ppi_len))-(T/T+1)*Anorm)**(T+1)
